...
def get_du_nd(pos,fe,n_dim,degrees=1):

    # generate grid of independent variables
    
    bins = pos.shape[0]

    mesh = create_mesh(pos,n_dim)
    ...
    shape = (n_dim,) + (len(pos_fine.T),) * n_dim
    force_matrix = np.zeros(shape)

    if n_dim > 1:

        mesh = create_mesh(pos_fine.T,n_dim)
        for i in range(n_dim):
            force_matrix[i] = force_funcs[i](mesh)
...
#(just works for 1D and 2D)

def get_du_2d(pos,fe,n_dim,kT=2.494,bounds= True,degrees=1):
    #fe*=kT
