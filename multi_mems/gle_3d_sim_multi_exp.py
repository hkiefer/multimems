...
@njit()
def integrate_gle_3_dim_exp(x0,v0, y0,ks, gammas, mass,nsteps, dt, kT,force_bins,force_matrix,fr_mode=1,uncorr_fr = False):
    ...
    for var in range(1,int(nsteps)):
        ...
        ff = dU_3D(xx,force_bins,force_matrix).reshape(n_dim,)
        ...
        ff = dU_3D(x1,force_bins,force_matrix).reshape(n_dim,)
        ...
        ff = dU_3D(x2,force_bins,force_matrix).reshape(n_dim,)
        ...
        ff = dU_3D(x3,force_bins,force_matrix).reshape(n_dim,)
...
@njit()
def dU_3D(x,force_bins,force_matrix):
    ...
    for i in range(len(x)):
        ...
        value[i] =  force_matrix[i][idxs[0]][idxs[1]][idxs[2]]
