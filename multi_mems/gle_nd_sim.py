...
@njit()
def integrate_gle_multi_dim_exp(x0,v0, y0,ks, gammas, mass,nsteps, dt, kT,force_bins,force_matrix,fr_mode=1,uncorr_fr = False):
    ...
    for var in range(1,int(nsteps)):
        ...
        ff = dU_multi(xx,force_bins,force_matrix).reshape(n_dim,)
        ...
        ff = dU_multi(x1,force_bins,force_matrix).reshape(n_dim,)
        ...
        ff = dU_multi(x2,force_bins,force_matrix).reshape(n_dim,)
        ...
        ff = dU_multi(x3,force_bins,force_matrix).reshape(n_dim,)
...
def dU_multi(x,force_bins,force_matrix):
    ...
    for i in range(len(x)):
        ...
        for j in range(len(x)):
            ...
            ##needed due to saving workaround of np.arrays in numba
            mask = np.arange(uu.shape[0], dtype=np.int64)
            uu = uu[np.where(mask== idxs[j])[0]]
            if uu.shape[0] == 1:
                uu = uu.reshape(uu.shape[1],1)
